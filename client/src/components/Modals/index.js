...
export function SaveModal(props) {
  return (
    <div>
      <ModalHeader className="saveDeal_MHeader">Save This Deal</ModalHeader>
      <ModalBody>
...
export function LoadModal(props) {
  return (
    <div>
      <ModalHeader className="loadDeal_MHeader">Load A Deal</ModalHeader>
      <ModalBody>
        {props.deals.length ? (
          <List>
            {props.deals.map(deal => (
              ...
              <ListItem key={deal._id}>
                <td><Button size="sm" className="load_Button mt-1 mb-1" id={deal._id} onClick={props.loadDeal} >Load</Button></td>
                <td>Deal: {deal.dealName} </td>
